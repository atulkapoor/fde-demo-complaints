...
**Check** — step and retry counts for the failing run against a normal one. A useful retry and an expensive loop are the same mechanism, with and without a stop condition.

**If it is this** — the fix is a bound or an escalation path, decided and written down -- not a model asked to please stop sooner.

## 4. The model -- only now
